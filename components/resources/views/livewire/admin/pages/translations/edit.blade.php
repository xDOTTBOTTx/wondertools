    ...
    <div class="card">
        <div class="card-body">
          ...
          <div class="alert alert-important alert-info" role="alert">
              <strong>{{ __('You are editing the :langNative version', ['langNative' => $lang_name]) }} (<a target="_blank" href="{{ ( $page_type == 'home' ) ? localization()->getLocalizedURL($locale, route('home') . '/', [], true) : localization()->getLocalizedURL($locale, route('home') . '/' . $slug, [], true) }}" class="text-light">{{ __('View page') }}</a>).</strong>
          </div>
            ...
            <form wire:submit.prevent="onEditPageTranslation">
                ...
                <div class="form-group">
                    <button class="btn btn-primary float-end" wire:loading.attr="disabled">
                        <span>
                            <div wire:loading.inline wire:target="onEditPageTranslation">
                                <x-loading />
                            </div>
