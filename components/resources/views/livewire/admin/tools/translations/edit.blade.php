    ...
    <div class="card">
        <div class="card-body">
          ...
          <div class="alert alert-important alert-info" role="alert">
              <strong>{{ __('You are editing the :langNative version', ['langNative' => $lang_name]) }} (<a target="_blank" href="{{ ( $page_type == 'home' ) ? localization()->getLocalizedURL($locale, route('home') . '/', [], true) : localization()->getLocalizedURL($locale, route('home') . '/' . $slug, [], true) }}" class="text-light">{{ __('View tool') }}</a>).</strong>
          </div>

            <form wire:submit.prevent="onEditToolTranslation">
                ...
                <div class="card mb-3 cursor-pointer">
                    ...
                    <div class="card-body">
                        ...
                        <span class="text-green">{{ ( $page_type == 'home' ) ? localization()->getLocalizedURL($locale, route('home') . '/', [], true) : localization()->getLocalizedURL($locale, route('home') . '/' . $slug, [], true) }}</span>
                ...
                <div class="form-group mb-3">
                    <label class="form-label">{{ __('Heading - Tool name') }}</label>
                ...
                <div class="form-group">
                    <button class="btn btn-primary float-end" wire:loading.attr="disabled">
                        <span>
                            <div wire:loading.inline wire:target="onEditToolTranslation">
                                <x-loading />
                            </div>
