        ...
        <div class="form-group">
            <button class="btn btn-info mx-auto d-block" wire:loading.attr="disabled">
                <span>
                    ...
                    <span wire:target="onRedirectChecker">{{ __('Check') }}</span>
                </span>
            </button>
        </div>
                           ...
                           <tr>
                               ...
                               <td>
                                    @switch($data['http_code'])
                                        @case(200)
                                             <span class="text-success fw-bold">{{ $data['http_code'] }}</span>
                                            @break
                                        @case(301)
                                        @case(302)
                                             <span class="text-warning fw-bold">{{ $data['http_code'] }}</span>
                                            @break
                                    
                                        @default
                                                <span class="text-danger fw-bold">{{ $data['http_code'] }}</span>
                                    @endswitch
                                </td>
                                <td>{{ $data['final_url'] }}</td>
                           </tr>
                   </tbody>
                </table>
            </div>
