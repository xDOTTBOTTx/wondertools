...
use App\Classes\WordToHtmlClass;
...
class WordToHtml extends Component
{
    ...
    {
        return view('livewire.public.tools.word-to-html');
    }
    ...
    public function onWordToHtml()
    {
        ...
        try {
            ...
            if ($this->isValidFile($filePath)) {
                ...
                $this->addError('error', __('The image must be a file of type: doc, docx.'));
            }
        } catch (\Exception $e) {
    ...
    private function validateInputs()
    {
        $validationRules = $this->convertType == 'remoteURL'
            ...
            : ['local_file' => 'required|mimes:doc,docx|file|max:' . 1024 * General::first()->file_size];
    ...
    private function processFile($filePath)
    {
        ...
        if (!empty($this->data)) {
            ...
            $this->dispatchBrowserEvent('showModal', ['id' => 'modalPreviewDownloadFile', 'download' => $dlLink]);
        }
    }
    ...
    private function saveHistory()
    {
        if (!empty($this->data)) {
            $history = new History;
            $history->tool_name = 'Word to HTML';
