...
use App\Classes\TextToPdfClass;
...
class TextToPdf extends Component
{
    ...
    {
        return view('livewire.public.tools.text-to-pdf');
    }
    ...
    public function onTextToPdf()
    {
        ...
        try {
            ...
            if ($this->isValidFile($filePath)) {
                ...
                $this->addError('error', __('The image must be a file of type: txt.'));
            }
        } catch (\Exception $e) {
    ...
    private function validateInputs()
    {
        $validationRules = $this->convertType == 'remoteURL'
            ...
            : ['local_file' => 'required|mimes:txt|file|max:' . 1024 * General::first()->file_size];
    ...
    private function isValidFile($filePath)
    {
        ...
        return in_array($fileExtension, ['txt']);
    }
